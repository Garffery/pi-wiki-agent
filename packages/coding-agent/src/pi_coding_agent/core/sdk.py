...
async def create_agent_session(
    ...
    session_manager = options.session_manager or SessionManager.create(cwd=cwd)   ## 创建一个session文件到电脑根目录下
    ...
    # Collect extension tools and wrap as AgentTool
    extension_tools: list[AgentTool] = []
    extensions_result_raw = resource_loader.get_extensions() if hasattr(resource_loader, "get_extensions") else None
    print(f"[SDK] extensions_result_raw type={type(extensions_result_raw)}, keys={extensions_result_raw.keys() if isinstance(extensions_result_raw, dict) else 'N/A'}")
    if extensions_result_raw:
        exts = extensions_result_raw.get("extensions", []) or []
        print(f"[SDK] 扩展数量: {len(exts)}")
        for ext in exts:
            ext_tools = getattr(ext, "tools", {}) or {}
            print(f"[SDK] 扩展 {ext.resolved_path}: tools={list(ext_tools.keys())}")
        for ext in (extensions_result_raw.get("extensions", []) or []):
            for tool_name, tool_def in (getattr(ext, "tools", {}) or {}).items():
                ext_execute = getattr(tool_def, "execute", None)
                ext_description = getattr(tool_def, "description", "")
                ext_parameters = getattr(tool_def, "parameters", {})
                ext_label = getattr(tool_def, "label", tool_name)
                if ext_execute is None:
                    continue
                def _make_wrapper(execute_fn):
                    async def _wrapper(tc_id, params, signal, on_update):
                        ...
                        try:
                            ...
                            return AgentToolResult(content=content_list, details=details)
                    ...
                    return _wrapper
                wrapped = _make_wrapper(ext_execute)
                extension_tools.append(AgentTool(
                    name=tool_name,
                    label=ext_label,
