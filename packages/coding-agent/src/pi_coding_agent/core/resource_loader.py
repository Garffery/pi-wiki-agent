...
class DefaultResourceLoader:
    ...
    async def reload(self) -> None:
        ...
        print(f"[DEBUG] resource_loader.reload() 被调用")
        self._path_metadata = {}
    ...
    async def _load_extensions(self) -> None:
        """
        Load extensions from settings paths + additional paths + discovery directories.
        ...
        print(f"[resource_loader] settings_manager={self._settings_manager}, _no_extensions={self._no_extensions}")
        if self._settings_manager:
            raw = self._settings_manager.get_extensions()
            print(f"[resource_loader] settings_manager.get_extensions()={raw}")
        print(f"[resource_loader] ext_paths={ext_paths}")
        ...
        if not ext_paths and not self._extension_factories:
            ...
            try:
                ...
                # load_extensions returns a LoadExtensionsResult dataclass, convert to dict
                base_result = {"extensions": base_result.extensions, "diagnostics": base_result.errors}
                ext_count = len(base_result.get("extensions", []))
                tool_count = sum(
                    len(getattr(ext, "tools", {}))
                    for ext in base_result.get("extensions", [])
                )
                print(f"[resource_loader] 加载了 {ext_count} 个扩展, 共 {tool_count} 个工具")
                # Detect conflicts
