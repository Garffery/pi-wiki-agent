...
Slash commands: /exit /clear /help /model /compact /thinking /session /tools
              Extensions may register additional slash commands.
...
async def run_tui(
    ...
    Set PI_FORCE_READLINE=1 to force readline mode.
    """
    # Check if readline mode is forced
    force_readline = os.environ.get("PI_FORCE_READLINE", "").strip() == "1"

    if force_readline or not sys.stdin.isatty() or not sys.stdout.isatty():
        from .mode import _run_readline_fallback
        await _run_readline_fallback(session, initial_messages)
        return

    try:
        await _run_pi_tui(session, initial_messages, extensions_result)
...
def _collect_extension_commands(extensions_result: Any) -> list[dict[str, str | None]]:
    """
    Collect slash commands from loaded extensions.
    ...
    for ext in extensions:
        ext_commands = getattr(ext, "commands", {}) or {}
        for cmd_name, cmd in ext_commands.items():
...
async def _run_pi_tui(
    ...
    # Build extension command handler map {name: handler}
    ext_command_handlers: dict[str, Any] = {}
    if extensions_result:
        for ext in (extensions_result.get("extensions", []) or []):
            for cmd_name, cmd in (getattr(ext, "commands", {}) or {}).items():
                handler = getattr(cmd, "handler", None)
                if handler and cmd_name not in ext_command_handlers:
                    ext_command_handlers[cmd_name] = handler
    ...
    for cmd_name, handler in ext_command_handlers.items():
        desc = ""
        if extensions_result:
            for ext in (extensions_result.get("extensions", []) or []):
                cmd_obj = getattr(ext, "commands", {}).get(cmd_name)
                if cmd_obj:
                    desc = getattr(cmd_obj, "description", "") or ""
                    break
        slash_commands.append(SlashCommand(name=cmd_name, description=desc))
    ...
    async def handle_submit(text: str) -> None:
        ...
        if stripped == "/help":
            ...
            # Add extension commands to help
            if ext_command_handlers:
                lines.append("")
                lines.append(bold("Extension commands:"))
                for cmd_name in ext_command_handlers:
                    desc = ""
                    for ext in (extensions_result.get("extensions", []) or []):
                        cmd_obj = getattr(ext, "commands", {}).get(cmd_name)
                        if cmd_obj:
                            desc = getattr(cmd_obj, "description", "") or ""
                            break
                    lines.append(f"  {cyan(f'/{cmd_name}')} — {desc}")
            append_history("\n".join(lines))
            tui.request_render()
            return
        ...
        if stripped.startswith("/"):
            cmd_name = stripped[1:].split(None, 1)[0]
            handler = ext_command_handlers.get(cmd_name)
            if handler:
                trace(f"handle_submit: extension command /{cmd_name}")
                args_part = stripped[len(f"/{cmd_name}"):].strip()
                # Capture stdout from extension handler to avoid garbling TUI
                ...
                try:
                    with contextlib.redirect_stdout(captured):
                        if asyncio.iscoroutinefunction(handler):
                            await handler(args_part, session)
                        else:
                            handler(args_part, session)
                    output = captured.getvalue()
                    if output:
                        append_history(output.rstrip("\n"))
                    else:
                        append_history(f"{dim(f'/{cmd_name} — completed (no output)')}")
                except Exception as exc:
                    append_history(f"{red(f'/{cmd_name} error:')} {exc}")
                    trace(f"handle_submit: /{cmd_name} exception={exc!r}")
                tui.request_render()
                return
