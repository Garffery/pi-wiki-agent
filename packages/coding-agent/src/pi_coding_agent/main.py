...
async def _run(args: Sequence[str]) -> int:
    ...
    migration_result = run_migrations(os.getcwd())   ## 版本兼容 可以先不看
    ...
    extensions_result = await load_extensions(ext_paths, os.getcwd(), event_bus)   ## 加载拓展系统
    ...
    if parsed.session and session_manager is None:
        return 1
    # Resolve model from parsed arguments
    model = None
    ...
    # Store extensions from session creation (includes settings-discovered extensions)
    session_extensions_result = result.extensions_result or extensions_result
    ...
    if parsed.resume:
        ...
        session = result.session
        session_extensions_result = result.extensions_result or session_extensions_result
    ...
    await run_interactive_mode(
        ...
        extensions_result=session_extensions_result,
    )
    return 0
