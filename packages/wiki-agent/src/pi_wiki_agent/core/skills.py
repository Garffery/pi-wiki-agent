"""Agent Skills loader for wiki-agent.

Mirrors pi_coding_agent.core.skills — discovers, validates, and loads skill files
from configured directories. Each skill is a markdown file with YAML frontmatter.
"""
...
def load_skills(options: LoadSkillsOptions | None = None) -> LoadSkillsResult:
    ...
    if include_defaults:
        add_skills(_load_skills_from_dir_internal(os.path.join(agent_dir, "skills"), "user", True))
        add_skills(_load_skills_from_dir_internal(os.path.join(cwd, CONFIG_DIR_NAME, "skills"), "project", True))
