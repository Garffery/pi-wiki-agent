...
class ProcessTerminal(Terminal):
    ...
    def _enable_raw_mode(self) -> None:
        ...
        if platform.system() == "Windows":
            # Windows-specific raw mode using msvcrt
            try:
                import msvcrt
                ...
                old_stdin_mode = ctypes.c_ulong()
                old_stdout_mode = ctypes.c_ulong()
                kernel32.GetConsoleMode(h_stdin, ctypes.byref(old_stdin_mode))
                kernel32.GetConsoleMode(h_stdout, ctypes.byref(old_stdout_mode))
                self._old_termios = (old_stdin_mode.value, old_stdout_mode.value)

                # Set new console modes
                # ENABLE_VIRTUAL_TERMINAL_INPUT (0x0200) for stdin
                # ENABLE_VIRTUAL_TERMINAL_PROCESSING (0x0004) | DISABLE_NEWLINE_AUTO_RETURN (0x0008) for stdout
                kernel32.SetConsoleMode(h_stdin, 0x0200)
                kernel32.SetConsoleMode(h_stdout, old_stdout_mode.value | 0x0004 | 0x0008)
                self._was_raw = True
            except Exception as e:
                # Fallback: try termios if available (e.g., in WSL or Git Bash)
                try:
                    import tty
                    import termios
                    fd = sys.stdin.fileno()
    ...
    def _disable_raw_mode(self) -> None:
        ...
        if not self._was_raw or self._old_termios is None:
            return
        ...
        if platform.system() == "Windows":
            try:
                ...
                old_stdin_mode, old_stdout_mode = self._old_termios
                kernel32.SetConsoleMode(h_stdin, old_stdin_mode)
                kernel32.SetConsoleMode(h_stdout, old_stdout_mode)
                self._old_termios = None
                self._was_raw = False
            except Exception:
                # Fallback to termios if available
                try:
                    import termios
                    fd = sys.stdin.fileno()
    ...
    def _setup_stdin_buffer(self) -> None:
        ...
        def _read_loop():
            import os as _os
            if platform.system() == "Windows":
                # Windows: use msvcrt.kbhit() instead of select
                import msvcrt
                import sys
